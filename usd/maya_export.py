...
def get_versions(filepath):
    version_folder = os.path.dirname(os.path.dirname(os.path.dirname(long)))
    file_name = os.path.splitext(cmds.file(q=True, sn=True).split('/')[-1])[0]
    version_number = file_name.split("_v")[-1]
    export_path = os.path.join(version_folder, "versions", file_name.split("_v")[0])
    print(export_path)
    return export_path, version_number

long = cmds.file(q=True, sn=True)
export_path, version_number = get_versions(long)
export_usd(os.path.join(export_path, "_v", version_number, ".usda"))